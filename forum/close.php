...
echo '<div class="munus"><div class="zag"><a href="/forum">Форум</a> | <a href="/forum/razd'.$razd['id'].'">'.$razd['name'].'</a> | <a href="/forum/podrazd'.$podrazd['id'].'">'.$podrazd['name'].'</a> | '.$thema['name'].'</div>';
 ...
 echo '<div class="block_menu">Вы действительно хотите <b>'.($thema['status'] == 1 ? 'открыть' : 'закрыть').'</b> тему?</div>
 <div class="news"><a href="/forum/thema'.$thema['id'].'/close?yes">Да</a> | <a href="/forum/thema'.$thema['id'].'">Отмена</a></div>';

   if(isset($_GET['yes']))
   {
  
DB::$dbs->query("UPDATE `forum_themes` set `status` = '".($thema['status'] == 0 ? '1' : '0')."' where `id` = '".$thema['id']."' limit 1");
$message = '<b>Тема '.($thema['status'] == 1 ? 'открыта' : 'закрыта').' для обсуждения!</b>';
