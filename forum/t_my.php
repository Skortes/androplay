...
$num = 10;  
$posts = DB::$dbs->querySingle("SELECT count(id) from `forum_themes` where `author` = ?",array($cms->us['id']));
...
$start = $page * $num - $num; 		
 ...
 $themes = DB::$dbs->query("SELECT * FROM `forum_themes` where `author` = ? order by `time` desc limit $start,$num",array($cms->us['id']));
