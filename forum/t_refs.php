...
verh('Обновленные темы');
 include '../system/funcs.php';


$num = 10;  
$posts = DB::$dbs->querySingle("SELECT count(id) from `forum_themes` where `last_time` > ?",array(time()-3600));
...
$start = $page * $num - $num; 		
 ...
 $themes = DB::$dbs->query("SELECT * FROM `forum_themes` where `last_time` > ? order by `last_time` desc limit $start,$num",array((time()-3600)));
